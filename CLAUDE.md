...
### Renomear categoria (v117)

Each category chip in Config → Categorias has a ✏️ (rename) button besides the × (delete). `editCat(t,i)` opens `#ov-editcat` pre-filled with the name and a hint of how many records use it (`contarCatUso`). `confirmarEditCat()` validates (non-empty, not a duplicate of another category of the same tipo — no merge), then rewrites the name in `catsR`/`catsE` **and** every record using the old name across all months/sources (same set as `contarCatUso`: `lancs` except `fat`, `ccLancs` when `t==='e'`, `fixos`, non-deleted `ocorrs`). Note `emo(cat)` is name-based, so renaming can change the displayed emoji.

### Excluir categoria (com reatribuição, v116)

`delCat(t,i)` opens modal `#ov-delcat` instead of deleting silently. `contarCatUso(t,cat)` counts every record using the category across all months: `db.lancs` (except `sub='fat'`, whose `cat` is the hardcoded 'Cartão de Crédito' regenerated by `syncFaturas`), `db.ccLancs` (only when `t==='e'`), `db.fixos`, and non-deleted override `db.ocorrs` (the edit override at ~line 1139 carries `cat`+`tipo`). If any records use it, deletion **requires reassigning** them to another category of the same tipo (`confirmarDelCat` rewrites `cat` on all of them at once, then splices the name out). Zero usage → simple confirm. No remaining category of that tipo → blocked (create one first). Result: no orphan categories are created going forward (existing orphans are still handled read-side by the Revisar screen).
